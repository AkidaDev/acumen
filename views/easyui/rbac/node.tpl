...
<script type="text/javascript">
    ...
    $(function(){
        $("#treegrid").treegrid({
            ...
            columns:[[
                ...
                {field:'Group__Id',title:'Packet',width:80,
    ...
    function addrow(){
        ...
        if(!vac.isEmpty(Row)){
            data[0].Pid =Row.Id;
            $("#treegrid").treegrid("expand",Row.Id);//Expand the node
            if($("#treegrid").treegrid("getLevel",Row.Id) >2){
                vac.alert("Allowed to add");
                return false;
            }
        }
        //如果没有数据，则从0行开始New
        ...
        $("#treegrid").treegrid("beginEdit",0);//Edit输入
    }
    //Edit
    function editrow(){
        var row = $("#treegrid").treegrid("getSelected");
    ...
    function saverow(){
        var row = $("#treegrid").treegrid("getSelected");
        if(!row){
            vac.alert("Please select the line you want to save");
            return;
        }
    ...
    function cancelrow(){
        var row = $("#treegrid").treegrid("getSelected");
        if(!row){
            vac.alert("Please select the line you want to cancel");
            return;
        }
    ...
    function delrow(){
        $.messager.confirm('Confirm','Are you sure you want to delete?',function(r){
            if (r){
                var row = $("#treegrid").treegrid("getSelected");
                if(!row){
                    vac.alert("Please select the row to be deleted");
                    return;
                }
